...
<template>
    ...
    <el-form :inline="true" :model="search.data" :rules="search.rules">
      ...
      <el-form-item label="状态">
        <el-select v-model="search.data.openState" placeholder="请选择" clearable style="width: 150px;">
          <el-option v-for="(item, index) in openStateOptions" :key="index" :label="item.label" :value="item.value"
      ...
      <el-form-item>
        ...
        <el-button type="primary" :icon="icons.Search">搜索</el-button>
      </el-form-item>
    </el-form>
  </div>
  <!-- 操作栏 -->
  ...
  <el-table :data="tabledata" style="width: 100%" border height="600px" table-layout="auto"
    ...
    <el-table-column fixed="right" label="操作" min-width="200" align="center">
      <template #default="scope">
        <el-button type="primary" size="small" @click="edit(scope.row)" :icon="icons.Edit">编辑</el-button>
        <el-button type="danger" size="small" @click="del(scope.row, scope.$index)" :icon="icons.Delete">删除</el-button>
      </template>
    </el-table-column>
  </el-table>
  <!-- 新增对话框 -->
...
<script>
import AddDialog from '@/components/company/store/store/addDialog.vue'
